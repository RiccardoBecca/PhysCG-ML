...
def load_params(csv_path: str) -> dict:
    ...
    for _, row in df.iterrows():
        ...
        params[bead] = {
            'C_rep': np.array(ast.literal_eval(str(row['C_rep']))),
            'b':     np.array(ast.literal_eval(str(row['b']))),
            'C_att': np.array(ast.literal_eval(str(row['C_att']))),
            'C':     np.array(ast.literal_eval(str(row['C'])))
        }
    return params
...
def run_cg_simulation(folded_pdb_file, unfolded_pdb_file, json_file, csv_file, charges_file=None, 
    ...
    charges = {}
    if charges_file:
        print(f"Loading bead charges from: {charges_file}")
    ...
    energy_expr = (
        "u_tt + u_dh;"
        "u_tt = (C_rep/r)^10 - f6 * (C_att/r)^6;"
        "f6 = 1.0 - exp(-x) * (1.0 + x + 0.5*x^2 + x^3/6.0 + x^4/24.0 + x^5/120.0 + x^6/720.0);"
        "x = b * r;"
        "u_dh = C_mix * 138.935456 * (q1 * q2) / (eps_r * r) * exp(-kappa * r);"
        "C_rep = sqrt(max(1e-12, (C_rep_01*C_rep_02 + C_rep_11*C_rep_12 + C_rep_21*C_rep_22)/3.0));"
        "b = sqrt(max(1e-12, (b_01*b_02 + b_11*b_12 + b_21*b_22)/3.0));"
        "C_att = sqrt(max(1e-12, (C_att_01*C_att_02 + C_att_11*C_att_12 + C_att_21*C_att_22)/3.0));"
        "C_mix = sqrt(max(1e-12, (C_01*C_02 + C_11*C_12 + C_21*C_22)/3.0));"
    )

    custom_nb = mm.CustomNonbondedForce(energy_expr)
    
    for param_name in ["C_rep_0", "C_rep_1", "C_rep_2", 
                        "b_0", "b_1", "b_2", 
                        "C_att_0", "C_att_1", "C_att_2", 
                        "C_0", "C_1", "C_2", "q"]:
        custom_nb.addPerParticleParameter(param_name)

    custom_nb.addGlobalParameter("eps_r", eps_r)
    custom_nb.addGlobalParameter("kappa", kappa_nm)
    ...
    for atom in unfolded_pdb.topology.atoms():
        ...
        c_rep_nm = (p['C_rep'] * c_rep_scale).tolist()
        b_nm = (p['b'] * 10.0).tolist()
        c_att_nm = (p['C_att'] * c_att_scale).tolist()
        c_nm = p['C'].tolist()
        q_val = float(charges.get(bead_name, 0.0))

        particle_params = c_rep_nm + b_nm + c_att_nm + c_nm + [q_val]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--charges', type=str, default=None, help="Path to bead_charges.json")
    ...
    run_cg_simulation(args.folded_pdb, args.unfolded_pdb, args.json, args.csv, args.charges, args.out, args.steps, args.temp, args.ionic)
