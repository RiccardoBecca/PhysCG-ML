...
mkdir -p logs
mkdir -p EXP
mkdir -p "EXP/${SYSTEM_NAME}"
mkdir -p "EXP/${SYSTEM_NAME}/analysis_plots"
...
CSV_PARAMS="data/potential/tt_dh_joint_params.csv"
CHARGES_JSON="data/potential/bead_charges.json"
OUTPUT_XYZ="EXP/${SYSTEM_NAME}/trajectory.xyz"
STEPS=20000000
...
python src/evaluate_CG_structure.py \
    ...
    --pdb "$FOLDED_PDB" \
    --outdir "EXP/${SYSTEM_NAME}/input_analysis"
...
python src/calculate_rmsd.py \
    --xyz "$OUTPUT_XYZ" \
    --pdb "$FOLDED_PDB" \
    --out "EXP/${SYSTEM_NAME}/analysis_plots/rmsd_trajectory.png"
...
python src/analyze_cg_folding.py \
    --xyz "$OUTPUT_XYZ" \
    --pdb "$FOLDED_PDB" \
    --out "EXP/${SYSTEM_NAME}/analysis_plots/folding_analysis.png"


# get Tica energy landscape
python src/run_tica_fel.py \
    --xyz "$OUTPUT_XYZ" \
    --out "EXP/${SYSTEM_NAME}/analysis_plots/tica_fel.png" \
    --eq_steps $((STEPS / 10))
